...
async function main() { // async function
  ...
  const contract2 = await hre.ethers.getContractFactory("staubTokenMint"); // get contract factory
  const c2 = await contract2.deploy(); // deploy contract
  await c2.deployed(); // wait for contract to be deployed
  console.log("staubTokenMInt deployed to:", c2.address); // log contract address
